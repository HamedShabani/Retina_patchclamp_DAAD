function sta=e_sta_sps(fpath,fname,fnamesps,jnk,nt,lock_out,trg_period,stim_file)
%% sta
...
% title('ISI Histogram for Indirect reponse with Burst Correction   ')
    ...
    for trg=1:length(stm)
        ...
        lbl=ones(length(data(data>=stm(idx(trg)) & (data<stm(idx(trg))+diffs))),1)*trg;
...
%     xlabel('time sec'); ylabel('Rate');
    ...
    print(fig,'-dpng',[fpath,'\Figures\','Electrical_sta ',fname(1:end-4),'  ',ch_name,' lt= ', num2str(lock_out),' No Burst Corrected .png'],'-r300')
