function [PSTH_all SPK_all]=single_cell_visualisze_nombar_nochirp_nocolor(fpath,fname,jnk,Stim_dur,bwid,cell_numb)
...
trgname(2).trigger='RawDat.trgss.BG';
...
%%
  ...
  count =[count;c];
  if isempty(psth)&contains(stm_name, 'BG')
        psth=zeros(1,(1+stim_dur)/bwid);
    elseif isempty(psth)&contains(stm_name, 'Chirp')
        psth=zeros(1,(5+stim_dur)/bwid);
    elseif isempty(psth)&contains(stm_name, 'Flash')
         psth=zeros(1,(stim_dur)/bwid)
  end
...
psth_chirp=zeros(1,(5+32)/bwid);
psth_color=zeros(1,(1+stim_dur)/bwid);

PSTH_all=[PSTH.('Flash') psth_chirp psth_color  ];
SPK_all=SPK_trg;
% figure
% hold on
% 
